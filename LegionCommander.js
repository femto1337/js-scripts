import { Game, MenuManager, EventsSDK, RendererSDK, Vector2, Color, Hero, ArrayExtensions, Utils, Vector3 } from 'wrapper/Imports';
let {
  MenuFactory
} = MenuManager;
  ...
  if (target !== undefined) {
    ...
    if (checkMods() && !target.IsMagicImmune) {
      ...
      if (amReflect.value && target.Name === 'npc_dota_hero_antimage' && !target.ModifiersBook.HasAnyBuffByNames(['modifier_silver_edge_debuff', 'modifier_viper_nethertoxin']) && target.GetAbilityByName('antimage_spell_shield').IsReady) {
        if (popLink(forcestaff, amReflectItems.selected_flags[3])) {
          return;
        }

        if (popLink(cyclone, amReflectItems.selected_flags[1])) {
          return;
        }

        if (popLink(atos, amReflectItems.selected_flags[2])) {
          return;
        }

        if (popLink(bolt, amReflectItems.selected_flags[5])) {
          return;
        }
        ...
        if (popLink(nullifier, amReflectItems.selected_flags[0])) {
          return;
        }
      ...
      if (popLinkV.value && target.HasLinkenAtTime()) {
        ...
        if (popLink(cyclone, popLinkItems.selected_flags[1])) {
          return;
        }

        if (popLink(atos, popLinkItems.selected_flags[2])) {
          return;
        }
        ...
        if (popLink(forcestaff, popLinkItems.selected_flags[4])) {
          return;
        }
        ...
        if (popLink(orchid, popLinkItems.selected_flags[6])) {
          return;
        }

        if (popLink(blood, popLinkItems.selected_flags[7])) {
          return;
        }

        if (popLink(seal, popLinkItems.selected_flags[8])) {
          return;
        }

        if (popLink(bolt, popLinkItems.selected_flags[9])) {
          return;
        }

        if (popLink(hurricane, popLinkItems.selected_flags[10])) {
          return;
        }
      ...
      if (useBlink() || aeonDispelling() || castAbility(shot, abils.selected_flags[1]) || useItem(atos, items.selected_flags[0]) || castAbility(seal, abils.selected_flags[2]) || useItem(veil, items.selected_flags[3]) || useItem(eblade, items.selected_flags[2]) || castAbility(bolt, abils.selected_flags[0]) || castAbility(flare, abils.selected_flags[3]) || useItem(orchid, items.selected_flags[5]) || useItem(dagon, items.selected_flags[4]) || useItem(blood, items.selected_flags[6]) || useItem(shiva, items.selected_flags[7])) {
        return;
      }
...
function CastToPrediction(near = false) {
  ...
  if (doubleFlare.value && sky.HasScepter) {
    ...
    } else sky.CastPosition(flare, IsFront(630, near));
...
EventsSDK.on("Update", cmd => {
  ...
  if (concShot.value && shot !== undefined) {
    ...
    let _f2 = hero => {
      return hero.Distance(sky) <= shot.CastRange && hero.IsAlive && hero.IsVisible;
    };
    ...
    cshotenemy = ArrayExtensions.orderBy(_r2, ent => {
      return ent.Distance(sky);
    })[0];
  }
});
EventsSDK.on("Draw", () => {
  ...
  if (drawTargetParticle.value) {
    ...
    if (targetParticle !== undefined) {
      if (nearest === undefined && target === undefined) {
        ...
        } else target.Position.toIOBuffer();
